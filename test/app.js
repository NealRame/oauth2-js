...
var oauth2_helpers = {
    ...
    /// Asynchronously associate an instance of your _User_ _model_ to a
    /// given _Open ID_ [_successful token response_](http://openid.net/specs/openid-connect-core-1_0.html#TokenResponse).
    ///
    /// _Parameters_:
    /// * `token`
    ///   An [Open ID successful token response]. According to how you have
    ...
    ///   - The second parameter is a hash object containing the following
    ///     attribute:
    ///     ```javascript
    ///     {
    ...
    find_user: function(token, callback) {
        callback(null, users[token.id_token.sub]);
    },
    /// ### `is_initialized(user)`
    ...
    ///   - The third parameter is a hash object containing the following
    ///     attribute:
    ///     ```javascript
    ///     {
    ///       token: { ... } // the passed token argument
    ...
    map_oauth2_user_info: function(params, oauth2_user_info, callback) {
        var user = {
            ...
            picture: oauth2_user_info.picture
        };
        callback(null, user);
    }
};
