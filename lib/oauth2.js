...
function OAuth2(params, helpers) {
    ...
    var helpers_ = _.clone(helpers);

    var callbackPath_ = url.parse(parameters_.redirect_uri).path;
    var logoutPath_ = path.join(path.dirname(callbackPath_), 'logout');

    var lookup_ = function(object, path) {
        return (function aux(object, path) {
            return ((typeof(object) === 'undefined'|| object === null || path.length === 0)
                ? object
                : aux(object[path[0]], path.slice(1)));
        })(
    ...
    var findUser_ = function(userId) {
        ...
        return new Promise(function(resolve, reject) {
            helpers_.findUser(userId, function(err, user) {
                if (err) {
                    reject(err);
                } else if (! user) {
    ...
    var isInitialized_ = function(user) {
        ...
        return new Promise(function(resolve, reject) {
            helpers_.isInitialized(user, function(err, initialized) {
                if (err) {
                    reject(err);
                } else {
    ...
    this.route = function() {
        ...
        router
            ...
            .all(function(req, res, next) {
                var state = lookup_(req, 'query.state');
                var csrf_token = lookup_(req, 'session.csrf_token');
