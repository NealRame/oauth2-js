...
function find_user(delegates, user_id) {
    debug(util.format('find_user: user_id=%j', user_id));
...
function init_user(delegates, user, token) {
    debug(util.format('init_user: user=%j, token=%j', user, token));
    return get_user_info(token.access_token)
        .then(function(oauth2_user_info) {
            return make_promise(delegates.mapUser, user, oauth2_user_info);
        });
}

function find_and_init_user(delegates, token) {
    debug(util.format('find_and_init_user: token=%j', token));
...
function OAuth2(params, helpers) {
    ...
    _.each(
        ['findUser', 'mapUser', 'isInitialized'],
        function(helper) {
    ...
    this.middleware = (function(req, res, next) {
        ...
        if (req.path !== this.callbackPath) {
            ...
            if (session && session.userId) {
                ...
                promise = createLoginLink_(session)
                    .then(function(link) {
                        return {
                            loggedIn: false,
                            loginLink: link
