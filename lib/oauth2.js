 ...
 *     - id, the user id,
 *     - callback, is following the node callback pattern.
 *
 *   - **`is_initialized`**: _Required_.
 *     A function taking a user and returning true if and only if the passed
 *     user object is considered initialized.
 *
 *   - **`map_oauth2_user_info`**: _Required_.
 *     A function taking two parameters:
 *     - an object containg open id user information,
 *     - callback, is following the node callbacl pattern.
 */
function OAuth2(params, helpers) {
    ...
    _.each(
        ['find_user', 'is_initialized', 'map_oauth2_user_info'],
        function(helper) {
    ...
    var getIDToken_ = function(code) {
        var params = _.extend(
            _.pick(parameters_, 'client_id', 'client_secret', 'redirect_uri'),
            {
                code: code,
                grant_type: 'authorization_code'
            }
        );
        return get_id_token(params);
    };

    var getLoginLink_ = function(state) {
        var params = _.extend(
    ...
    var getUserInfo_ = function(token) {
    ...
    var initUser_ = function(user, token) {
        return getUserInfo_(token)
            .then(function(oauth2_user_info) {
                return new Promise(
                    function(resolve, reject) {
                        helpers.map_oauth2_user_info(
                            oauth2_user_info,
                            function(err, mapped_user) {
                                if (err) {
                                    reject(err);
                                } else {
                                    resolve(_.extend(user, mapped_user));
                                }
                            }
                        )
                    }
    ...
    this.route = function() {
        ...
        var callback_path = url.parse(parameters_.redirect_uri).path;

        router
            .route(path.join(path.dirname(callback_path), 'login'))
        ...
        router
            .route(path.join(path.dirname(callback_path), 'logout'))
            .get(function(req, res) {
        ...
        router
            ...
            .get(function(req, res) {
                var session = req.session;
                var user_tokens = null;

                getIDToken_(req.query.code)
                    .then(function(tokens) {
                        user_tokens = tokens;
                        delete session.csrf_token;
                        return findUser_(tokens.id_token.sub);
                    })
                    .then(function(user) {
                        if (user) {
                            if (helpers.is_initialized(user)) {
                                return user;
                            }
                            return initUser_(user, user_tokens.access_token);
                        }
                        throw _.extend(
                            new Error('Unauthorized'), {status: 403}
                        );
